...
class MeetboutViewSet(rest.AtlasViewSet):
    ...
    Meetbouten worden, op aanwijzing van het stadsdeel, met een zekere
    regelmaat geplaatst in verschillende panden. In
    gebieden die naar verwachting meer met funderingsproblemen te maken
    krijgen, worden extra meetbouten geplaatst. De verkregen meetgegevens
    zijn onderling vergelijkbaar. Er zijn ongeveer 13.000 bouten geplaatst.
    Het plaatsen van een meetbout heeft nagenoeg geen gevolgen voor het pand.
    Er wordt slechts een klein gaatje in een gevelsteen geboord.
    """
...
class MetingViewSet(rest.AtlasViewSet):
    ...
    nulmeting, is het uitgangspunt voor het beoordelen van eventuele
    deformatie (zakking). In principe zijn sindsdien drie herhalingsmetingen
    uitgevoerd. Het verschil tussen de nulmeting en de herhalingsmeting
    is een maat voor het zettingsgedrag. Soms vervallen meetbouten.
    Deze zijn om de een of andere reden niet meer aanwezig.
    """
...
class ReferentiepuntViewSet(rest.AtlasViewSet):
    ...
    Voor de meting van meetbouten zijn zogenaamde referentiepunten
...
class RollaagViewSet(rest.AtlasViewSet):
    ...
    Om de zakking van een heel bouwblok te bepalen worden
    rollagen gemeten. Een rollaag is een herkenbare laag in de
    bebouwing. Dit kan een doorlopende voeg zijn of een ander
    herkenbaar bouwkundig element.

    Op de plaats van de pandscheidingen wordt de hoogte van deze
    rollaag gemeten. Hierdoor ontstaat er een overzicht
