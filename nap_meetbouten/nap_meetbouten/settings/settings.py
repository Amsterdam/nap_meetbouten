...
from nap_meetbouten.settings.settings_databases import (
    LocationKey,
    get_docker_host,
    get_database_key,
...
PARTIAL_IMPORT = dict(numerator=0, denominator=1)
...
BATCH_SETTINGS = dict(batch_size=500)
...
SWAGGER_SETTINGS = {
    ...
    "base_path": "acc.api.data.amsterdam.nl/meetbouten/docs"
    if DEBUG
    else "127.0.0.1:8000/meetbouten/docs",  # noqa
    "info": {
...
if not os.path.exists(DIVA_DIR):
    ...
    print("Geen lokale DIVA bestanden gevonden, maak gebruik van testset", DIVA_DIR, "\n")
...
if SENTRY_DSN:
    sentry_sdk.init(
        dsn=SENTRY_DSN, environment="nap_meetbouten", integrations=[DjangoIntegration()]
    )
