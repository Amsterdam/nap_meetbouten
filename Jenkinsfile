...
node {
    ...
    stage('Test') {
        tryStep "test", {
            withCredentials([[$class: 'StringBinding', credentialsId: 'GOB_OBJECTSTORE_PASSWORD', variable: 'GOB_OBJECTSTORE_PASSWORD']]) {
    ...
    stage("Build image") {
        tryStep "build", {
                ...
                def image = docker.build("datapunt/nap:${env.BUILD_NUMBER}")
                image.push()
            }
        }
    }
    ...
    node {
        stage('Push acceptance image') {
            tryStep "image tagging", {
                docker.withRegistry("${DOCKER_REGISTRY_HOST}",'docker_registry_auth') {
                    def image = docker.image("datapunt/nap:${env.BUILD_NUMBER}")
                    image.pull()
        ...
        stage("Deploy to ACC") {
            tryStep "deployment", {
                ...
                parameters: [
                    ...
                    [$class: 'StringParameterValue', name: 'PLAYBOOKPARAMS', value: "-e cmdb_id=app_nap"],
                ]
            }
        }
    }
